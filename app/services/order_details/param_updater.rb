...
class OrderDetails::ParamUpdater
  ...
  def update_param_attributes(params)
    ...
    @order_detail.transaction do
      ...
      if @order_detail.reservation.present?
        @order_detail.reservation.save_as_user(@editing_user) || raise(ActiveRecord::Rollback)
      end
  ...
  def assign_price_changed_by_user
    ...
    elsif %w[actual_cost actual_subsidy price_change_reason].intersect?(@order_detail.changed)
  ...
  def cost_params(params)
    return params.slice(:actual_cost, :actual_subsidy).permit! unless SettingsHelper.feature_on?("roles.allow_global_billing_admin_update_actual_prices")
