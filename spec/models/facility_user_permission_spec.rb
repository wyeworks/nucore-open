...
RSpec.describe FacilityUserPermission do
  ...
  subject(:permission) { described_class.new(user: user, facility: facility) }

  it { is_expected.to be_valid }

  it "enforces uniqueness of user per facility" do
    create(:facility_user_permission, user: user, facility: facility)
  ...
  it "allows the same user in different facilities" do
    ...
    create(:facility_user_permission, user: user, facility: other_facility)
